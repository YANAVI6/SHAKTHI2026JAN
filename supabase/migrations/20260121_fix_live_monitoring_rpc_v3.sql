-- Fix Live Monitoring RPC - Final Type Casting Fix
-- Resolves: assigned_employee_id is TEXT but needs to be compared with UUID
...
BEGIN
  ...
  -- 3. Calculate detailed stats per telecaller
  telecaller_stats AS (
    SELECT 
      tt.telecaller_id,
      tt.telecaller_name,
  ...
  -- FIX: Cast assigned_employee_id to UUID for comparison
  total_cases_per_telecaller AS (
    SELECT 
      assigned_employee_id::uuid as telecaller_id,
      team_id,
    ...
    GROUP BY assigned_employee_id::uuid, team_id
  ),
