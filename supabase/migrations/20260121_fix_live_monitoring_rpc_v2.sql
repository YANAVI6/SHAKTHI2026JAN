-- Fix Live Monitoring RPC - Type Casting Fix
-- Resolves: operator does not exist: uuid = text
...
BEGIN
  ...
  WITH target_telecallers AS (
    ...
    WHERE t.tenant_id = p_tenant_id
      ...
      AND e.status = 'active'
      AND e.role::text = 'Telecaller'  -- Explicit cast to text
  ),
  ...
  total_cases_per_telecaller AS (
    SELECT 
      assigned_employee_id as telecaller_id,
      team_id,
      COUNT(*) as total_cases
    FROM customer_cases
    WHERE tenant_id = p_tenant_id
      AND team_id = ANY(p_team_ids)
      AND case_status::text != 'deleted'  -- Explicit cast to text
    GROUP BY assigned_employee_id, team_id
  ),
