-- Fix Live Monitoring RPC to use Asia/Kolkata Timezone for "Today"
-- This prevents issues where server UTC time might misclassify "Today" vs "Yesterday" for IST users.
...
DECLARE
  ...
  today_date DATE;
BEGIN
  -- Determine "Today" in Asia/Kolkata timezone
  ...
  -- 2. Get calls made TODAY (IST) by these telecallers
  today_calls AS (
    SELECT 
      ccl.case_id,
      ccl.employee_id,
    ...
    WHERE (ccl.created_at AT TIME ZONE 'Asia/Kolkata')::date = today_date
    ...
    SELECT 
      ...
      JSON_AGG(
        JSON_BUILD_OBJECT(
          ...
          'lastCallTime', to_char(tc.created_at AT TIME ZONE 'Asia/Kolkata', 'HH12:MI:SS AM'), -- Format in IST
          'callCount', (
             SELECT COUNT(*) 
  ...
  -- FIX: Changed from '24 hours' to 'today_date' (IST)
  user_activity_latest AS (
    ...
    FROM user_activity
    WHERE tenant_id = p_tenant_id
      AND (last_active_time AT TIME ZONE 'Asia/Kolkata')::date = today_date
