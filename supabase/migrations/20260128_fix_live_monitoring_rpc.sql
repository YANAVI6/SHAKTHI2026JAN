-- Fix Live Monitoring RPC to strictly use "Today" for status
-- Previous version looked at "last 24 hours" which caused yesterday's late workers to appear Online today
...
BEGIN
  ...
  -- FIX: Changed from '24 hours' to 'today_start'
  user_activity_latest AS (
    ...
    FROM user_activity
    WHERE tenant_id = p_tenant_id
      AND last_active_time >= today_start  -- CRITICAL FIX HERE
